...
if ($logged_in=="yes") {
    ...
    */
    ...
    <?php  if ($posted==1){ ?>
	    <center>
	    <IMG SRC="graph_pie.php?min_call=<?php echo $min_call?>&fromstatsday_sday=<?=$fromstatsday_sday?>&months_compare=<?=$months_compare?>&fromstatsmonth_sday=<?=$fromstatsmonth_sday?>&dsttype=<?=$dsttype?>&sourcetype=<?=$sourcetype?>&clidtype=<?=$clidtype?>&channel=<?=$channel?>&resulttype=<?=$resulttype?>&dst=<?=$dst?>&src=<?=$src?>&clid=<?=$clid?>&userfieldtype=<?=$userfieldtype?>&userfield=<?=$userfield?>&accountcodetype=<?=$accountcodetype?>&accountcode=<?=$accountcode?>" ALT="Stat Graph">
	    </center>
    <?php  } ?>
    </center>
    <br/><br/>
		    ...
		    <table class="bar-status" width="100%" border="0" cellspacing="1" cellpadding="2" align="center">
			    ...
			    <tr>
            		<td align="left" bgcolor="#000033">					
					    <font size="1" color="#ffffff"><b>Select the Month</b></font>
				    </td>
